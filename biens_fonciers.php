...
<title>Biens Fonciers — TUNISAIR</title>
...
.nav-brand-text { font-size: 15px; font-weight: 700; color: var(--red); letter-spacing: .01em; line-height: 1.2; }
.nav-right { display: flex; align-items: center; gap: 20px; }
.nav-user { font-size: 13.5px; font-weight: 500; color: var(--muted); }
.btn-deconnexion {
  background: var(--red); color: white; padding: 9px 22px; border-radius: 8px;
  text-decoration: none; font-size: 13.5px; font-weight: 600; letter-spacing: .01em;
  transition: background .2s, transform .15s, box-shadow .2s;
  box-shadow: 0 3px 10px rgba(200,16,46,.25);
}
.btn-deconnexion:hover { background: var(--red-dark); transform: translateY(-1px); box-shadow: 0 5px 16px rgba(200,16,46,.35); }
...
.page {
  ...
  padding: 40px 20px;
  background:
    radial-gradient(ellipse 65% 55% at 10% 15%, rgba(200,16,46,.06) 0%, transparent 65%),
    radial-gradient(ellipse 55% 50% at 90% 85%, rgba(29,78,216,.07) 0%, transparent 65%),
    var(--bg);
}
...
.card {
  ...
  animation: rise .6s cubic-bezier(.22,.68,0,1.2) both;
}
@keyframes rise {
...
.btn-retour-login svg { flex-shrink: 0; transition: transform .15s; }
...
.btn-retour-login:hover svg { transform: translateX(-3px); }
...
.choice {
  ...
  transition: transform .22s cubic-bezier(.22,.68,0,1.3), box-shadow .22s ease;
}
...
.arrow {
  ...
  transition: background .2s, transform .22s cubic-bezier(.22,.68,0,1.3);
}
  ...
  <div class="card">
    ...
    <div class="card-top">
      <div class="breadcrumb">
        ...
        <span>Biens Fonciers</span>
      </div>

      <a href="login.php" class="btn-retour-login">
    ...
    <h1 class="card-title">Biens <span>Fonciers</span></h1>
    <p class="card-sub">Sélectionnez la zone géographique pour consulter le portefeuille  de biens correspondant.</p>

    <div class="divider"></div>

    <div class="choices">

      <a href="biens_liste.php?zone=Tunisie" class="choice choice-tn">
        <div class="flag-wrap"><span class="flag-text">TN</span></div>
        <div class="choice-body">
          <div class="choice-label">Tunisie</div>
          <div class="choice-meta">Marché national · biens locaux</div>
        </div>
        <div class="arrow">
      ...
      <a href="biens_liste.php?zone=Etranger" class="choice choice-int">
        <div class="flag-wrap">🌍</div>
        <div class="choice-body">
          <div class="choice-label">Étranger</div>
          <div class="choice-meta">Marché international · biens à l'étranger</div>
        </div>
        <div class="arrow">
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            <path d="M3 8h10M9 4l4 4-4 4" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/>
